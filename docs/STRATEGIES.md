...
Every strategy submits exits through `place_order_via_bridge → OrderRouter`. The router calls `PositionOwnershipStore.try_acquire`, which now treats **OWNED → RELEASING as an exclusive lock**: a second exit-side acquire on a contract whose record is already in RELEASING is refused with `PositionOwnershipDecision(allowed=False, reason="exit_already_in_flight")` until either (a) the broker confirms terminal fill (entry flattens, lock auto-releases) or (b) the watchdog window expires (env `POSITION_OWNERSHIP_EXIT_LOCK_MAX_SECONDS`, default 90s — logs `exit_lock_watchdog_expired` WARNING then permits the retry so ops surfaces the stuck lock instead of the position being permanently bricked). Once a terminal fill flattens ownership, a stale strategy/system exit for the same known contract is rejected with `no_owned_position_to_exit`; a new exit is allowed only after a fresh entry or broker-reconciled UNKNOWN position exists. Explicit system-exit bypass also fails closed when the owner retry cannot acquire the lock. Protection is strategy-independent — `ema20_strategy`, `exclusive_nifty_ce_buy`, `put_momentum_scalper`, `nifty_weekly_credit_spreads`, and any future strategy benefit without per-strategy code. Watch: `exit_already_in_flight` and `exit_without_owned_position_blocked` log lines. See [issue #200](https://github.com/abhipharmatiwari-netizen/Phoenix/issues/200), [issue #317](https://github.com/abhipharmatiwari-netizen/Phoenix/issues/317), and `tests/orders/test_position_ownership_exit_lock.py`.
...
Cuts across all strategies via `PositionTrailingLockEngine` (env: `POSITION_TRAILING_LOCK_ENABLED=true`). Independent of strategy-level trail logic; tracks each open position's peak unrealised P&L and exits when it falls below `peak × (1 − giveback_pct)` (default 10%) once peak ≥ ₹500. The default path runs from the hub profit watchdog (`HUB_SUBSCRIPTION_POLL_INTERVAL`, clamped to a 10s minimum); optional tick-driven evaluation can also be enabled with `POSITION_TRAILING_LOCK_TICK_ENABLED=true`. Tick mode evaluates every registered hub account runner, including multiple SHADOW runners; shadow strategies that need independent locks must retain separate broker-account position views rather than collapsing into one account+symbol net position. See commit `29c24f0`.
...
- **Issues [#307](https://github.com/abhipharmatiwari-netizen/Phoenix/issues/307) / [#311](https://github.com/abhipharmatiwari-netizen/Phoenix/issues/311) / [#316](https://github.com/abhipharmatiwari-netizen/Phoenix/issues/316)** — dashboard status now includes readiness, position-authority blockers, and readiness-critical alerts so the operator UI cannot show green when readiness is red. Deployed in `349d55f` on 2026-05-20.
- **Issue [#308](https://github.com/abhipharmatiwari-netizen/Phoenix/issues/308)** — added audited broker-flat recovery evidence at `GET /admin/state/position-authority/recovery` and documented the safe clear workflow in `docs/runbooks/position_authority_recovery.md`. Two broker-flat `RECOVERY_PENDING` records were cleared with `force=false` during deployment. Deployed in `349d55f` on 2026-05-20.
- **Issue [#310](https://github.com/abhipharmatiwari-netizen/Phoenix/issues/310)** — dashboard health now exposes OI/ML shadow ingestion row counts, validation-report freshness, dry-run invariants, and degraded alerting for missing/stale shadow ingestion evidence. Deployed in `349d55f` on 2026-05-20.
- **Issues [#312](https://github.com/abhipharmatiwari-netizen/Phoenix/issues/312) / [#313](https://github.com/abhipharmatiwari-netizen/Phoenix/issues/313) / [#314](https://github.com/abhipharmatiwari-netizen/Phoenix/issues/314)** — dashboard WebSocket/admin/control-tower payloads are entitlement scoped, and position/PnL normalization uses the same invalid-mark policy across REST/WebSocket/frontend paths. Deployed in `349d55f` on 2026-05-20.
- **Issue [#315](https://github.com/abhipharmatiwari-netizen/Phoenix/issues/315)** — added focused contract/regression tests covering readiness status, alert visibility, entitlement scoping, position normalization, broker-flat recovery, system-exit attribution, and OI/ML shadow ingestion evidence. Deployed in `349d55f` on 2026-05-20.
