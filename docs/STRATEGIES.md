...
Cuts across all strategies via `PositionTrailingLockEngine` (env: `POSITION_TRAILING_LOCK_ENABLED=true`). Independent of strategy-level trail logic; tracks each open position's peak unrealised P&L and exits when it falls below `peak × (1 − giveback_pct)` (default 10%) once peak ≥ ₹500. The default path runs from the hub profit watchdog (`HUB_SUBSCRIPTION_POLL_INTERVAL`, clamped to a 10s minimum); optional tick-driven evaluation can also be enabled with `POSITION_TRAILING_LOCK_TICK_ENABLED=true`. Tick mode evaluates every registered hub account runner, including multiple SHADOW runners; shadow strategies that need independent locks must retain separate broker-account position views rather than collapsing into one account+symbol net position. See [`feat(pnl): per-position trailing profit lock`](https://github.com/abhipharmatiwari-netizen/Phoenix/commit/e960ff4).
