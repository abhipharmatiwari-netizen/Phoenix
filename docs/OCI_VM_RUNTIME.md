...
Last verified: 2026-06-06 13:28 UTC from the running OCI VM.
...
| Active git commit/branch | `main`, `ce837e8...` plus live operator config drift | `git -C /opt/phoenix/app branch --show-current`, `rev-parse HEAD`, mounted config/env evidence | VM checkout is on `main` at `ce837e8...`; `/opt/phoenix/app/app/config/strategy_env.yaml` and `/opt/phoenix/phoenix-deploy.env` remain operator-owned runtime inputs | Deploy env image tag is `local-ce837e8`; backend and nginx have been restarted/reconciled against that tag |
...
| Backend image | `phoenix-local-backend:local-ce837e8` | `docker inspect phoenix-oci-backend` | Local image, not OCIR | Source bind mounts are still active |
| Web image | `phoenix-local-nginx:local-ce837e8` | `docker inspect phoenix-oci-web` | Local image, not OCIR | Public nginx `/readyz` and `/health/summary` proxy to redacted backend endpoints; `/manifest.json`, `/favicon.svg`, and `/favicon.ico` are served as static assets; stale `/static/*` assets return 404 instead of SPA HTML; frontend runtime failures render a visible recovery screen instead of a blank root |
