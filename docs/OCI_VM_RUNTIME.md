...
Last verified: 2026-06-06 11:05 UTC from the running OCI VM.
...
| Active git commit/branch | `main`, `158549e...` plus live operator config drift | `git -C /opt/phoenix/app branch --show-current`, `rev-parse HEAD`, mounted config/env evidence | VM checkout is on `main` at `158549e...`; `/opt/phoenix/app/app/config/strategy_env.yaml` and `/opt/phoenix/phoenix-deploy.env` remain operator-owned runtime inputs | Deploy env image tag is `local-158549e`; backend and nginx have been restarted/reconciled against that tag |
...
| Backend image | `phoenix-local-backend:local-158549e` | `docker inspect phoenix-oci-backend` | Local image, not OCIR | Source bind mounts are still active |
| Web image | `phoenix-local-nginx:local-158549e` | `docker inspect phoenix-oci-web` | Local image, not OCIR | Public nginx `/readyz` and `/health/summary` proxy to redacted backend endpoints; `/manifest.json`, `/favicon.svg`, and `/favicon.ico` are served as static assets; stale `/static/*` assets return 404 instead of SPA HTML |
