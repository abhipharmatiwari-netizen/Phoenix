  ...
  return (
    <div style={{
      ...
      <div style={{
        ...
        maxWidth: 420, width: '90%', boxShadow: '0 4px 12px rgba(0,0,0,0.15)',
      }}>
        ...
        {requireTyped && (
          <div style={{ margin: '0.75rem 0' }}>
            ...
            <input
              ...
              style={{ width: '100%', padding: '0.5rem', border: '1px solid #d1d5db', borderRadius: 4, marginTop: 4 }}
              aria-label={`Type ${requireTyped} to confirm`}
            />
          </div>
        )}
        <div style={{ display: 'flex', gap: '0.5rem', justifyContent: 'flex-end', marginTop: '1rem' }}>
          <button onClick={onCancel} style={{ padding: '0.5rem 1rem', border: '1px solid #d1d5db', borderRadius: 4, cursor: 'pointer', backgroundColor: '#fff' }}>
            Cancel
          </button>
          <button
            onClick={onConfirm} disabled={!canConfirm}
            style={{
              padding: '0.5rem 1rem', border: 'none', borderRadius: 4, cursor: canConfirm ? 'pointer' : 'not-allowed',
